...
function Servipag() {
	...
	this.ordencomfircompra = config.ordencomfircompra;
	this.firma = '';
	this.xml1 = [];
...
Servipag.prototype.validarXml4 = function (xml) {
	...
	for (var i in this.xml4) {
		...
		for (var j in this.ordencomfircompra) {
			if (xml4.hasOwnProperty(this.ordencomfircompra[j])) {
...
Servipag.prototype.desencriptar = function (cadena, firma) {
	...
	var resultado = llave.verify('md5', cadena, firma, 'base64');
 ...
 */
exports.create = function(rutaLlavePrivada, rutaLlavePublica) {
	return new Servipag(rutaLlavePrivada, rutaLlavePublica);
};